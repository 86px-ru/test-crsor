...
const BlinkingAsciiDots = ({
  ...
  const animate = useCallback(() => {
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        if (Math.abs(waveValue) > 0.1) {
          // Threshold to create some empty space (lowered for more visibility)
          const charIndex = Math.floor(normalizedValue * CHARS.length)
          const char = CHARS[Math.min(CHARS.length - 1, Math.max(0, charIndex))]

          // Calculate opacity based on wave value (increased for better visibility)
          const opacity = Math.min(0.95, Math.max(0.5, 0.5 + normalizedValue * 0.45))
  ...
  useEffect(() => {
    ...
    // Initial resize with delay to ensure container is mounted
    setTimeout(() => {
      handleResize()
    }, 100)
...
export { BlinkingAsciiDots as DelicateAsciiDots }
